...
def get_live_price(symbol):
    """Récupère le cours exact en direct via Yahoo Finance."""
    try:
...
def analyze_market_real(symbol):
    """Analyse technique rigoureuse basée sur les données réelles RSI & MACD."""
    try:
        ticker = yf.Ticker(symbol)
        data = ticker.history(period="1d", interval="1m")
        if len(data) >= 20:
            ...
            # Calcul réel de la confiance basé sur l'éloignement du RSI
            if rsi <= 30:
                ...
                return "CONSEIL : NE PAS TRADER ⚪", f"Marché neutre/indécis (RSI {round(rsi, 1)})", 40.0
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🏁 Bilan de la Session", callback_data="end_session")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    text = (
        "🎯 **BOT JOE GEM - PROTOCOLE RÉEL** 🎯\n\n"
        "Toutes les analyses et résultats sont calculés en direct sur les cours officiels du marché.\n\n"
        "Choisissez votre catégorie d'actifs :"
    )
    if update.message:
...
async def show_category_assets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"📊 **Catégorie : {category}**\nSélectionnez l'actif à analyser en temps réel :",
...
async def select_timeframe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"🎯 **Actif :** `{asset_name}`\nChoisissez la durée d'expiration du trade :",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
...
async def generate_signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Message de préparation / Alerte avant bougie
    await query.edit_message_text(
        f"🔍 **ANALYSE DU MARCHÉ EN COURS (`{asset_name}`)...**\n"
        f"⏳ *Calcul des indicateurs et attente des 5 secondes avant l'ouverture de la bougie...*",
        parse_mode="Markdown"
    )

    # Récupération de l'analyse réelle
    direction, rationale, confidence = analyze_market_real(symbol)
    
    # Pause technique pour caler l'entrée
    await asyncio.sleep(3)

    # 2. Récupération du prix d'entrée EXACT en direct
    entry_price = get_live_price(symbol)
    if not entry_price:
        await query.message.reply_text("❌ **Erreur :** Impossible de récupérer le cours en direct (marché fermé ou indisponible).")
        return

    # Message de confirmation d'entrée en trade
    signal_msg = (
        f"🚨 **SIGNAL D'ENTRÉE MENTALE JOE GEM** 🚨\n\n"
        f"📊 **Actif :** `{asset_name}`\n"
        f"🎯 **Direction :** **{direction}**\n"
        f"📍 **Prix d'Entrée Réel :** `{entry_price}`\n"
        f"⏱ **Expiration :** **{exp_label}**\n\n"
        f"🛠 **Raison Technique :** `{rationale}`\n"
        f"🔥 **Indice de Confiance Réel :** **{confidence}%**\n\n"
        f"⏳ *Trade en cours sur le marché réel...*"
    )
    msg_obj = await query.message.reply_text(signal_msg, parse_mode="Markdown")

    # 3. Suivi dynamique du temps d'expiration avec barre de chargement
    ...
    while elapsed < total_seconds:
        ...
        try:
            await msg_obj.edit_text(
                ...
                f"⏱ **Temps Restant :** `{max(0, total_seconds - elapsed)}s`\n"
    ...
    # 4. Clôture stricte et vérification du résultat réel
    ...
    if res_type == "WIN":
        result_text = "✅ **RÉSULTAT RÉEL : WIN 🟢** (Le cours a respecté la direction)"
    elif res_type == "LOSS":
        result_text = "❌ **RÉSULTAT RÉEL : LOSS 🔴** (Le cours a inversé la direction)"
    else:
        result_text = "⚪ **RÉSULTAT RÉEL : DOJI ⚪** (Prix identique à l'entrée)"
    ...
    keyboard = [
        [InlineKeyboardButton("🔄 Choisir un autre Actif", callback_data="back_to_start"),
         InlineKeyboardButton("🏁 Clôturer la Session", callback_data="end_session")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
